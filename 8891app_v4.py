...
def find_car():
    ...
    try:
        if entry0.get() == '':
            ...
            return
        labelstart.config(text='開啟隱藏網頁，請稍候！',fg = '#7cfc00')
        labelstart.update_idletasks()
        url = 'https://auto.8891.com.tw'
        chrome = get_chrome(url,hide=True)
        time.sleep(1)
        add_close = find_element(chrome,'/html/body/div[5]/div/div[6]/div[2]/div/div[7]/div[4]')
        if add_close:
            add_close.click()
        time.sleep(1)
        ...
        if tkmessage.askquestion(title = '警告', message = f'{key}:{pages}頁(確定執行？(yes/no):)') == 'yes':
            labelstart.config(text='爬找中，請稍候....',fg = '#b0e0e6')
            labelstart.update_idletasks()
            start=datetime.now()
...
def select_car():
    ...
    try:
        ...
        analyze_car_data()
        if year != '' and price != '':
            df1=sort_by_year_price(year, price) 
        elif year == '':
            df1=sort_by_price(price)
        elif price == '':
            df1=sort_by_year(year)
        df1['year']=df1['year'].apply(lambda x : str(x)+'年')
        df1['price']=df1['price'].apply(lambda x: str(x/10000)+'萬')
...
def to_png():
    ...
    try:
        ...
        df['price']=df['price'].apply(lambda x:(eval(x.replace('萬',''))))
...
def sort_by_year_price(year,price):
    if price == price_list[0] and year == year_list[0]:
        return df[df['year']<=2000].sort_values(['price'],ascending = False)
    elif price == price_list[1] and year == year_list[0]:
        return df[df['year']<=2000].sort_values(['price'])
    elif price == price_list[0]:
        return df[df['year']==eval(year)].sort_values(['price'],ascending = False)
    elif price == price_list[1]:
        return df[df['year']==eval(year)].sort_values(['price'])
    elif price == price_list[2] and year == year_list[0]:
        return df[(df['year']<=2000) & (df['price']<= 100000)]
    elif price == price_list[2]:
        return df[(df['year']==eval(year)) & (df['price']<= 100000)]
    elif price == price_list[3] and year == year_list[0]:
        return df[(df['year']<=2000) & (df['price']<= 500000)]                
    elif price == price_list[3]:
        return df[(df['year']==eval(year)) & (df['price']<= 500000)]
    elif price == price_list[4] and year == year_list[0]:
        return df[(df['year']<=2000) & (df['price']<= 1000000)]                
    elif price == price_list[4]:
        return df[(df['year']==eval(year)) & (df['price']<= 1000000)]
    elif price == price_list[5] and year == year_list[0]:
        return df[(df['year']<=2000) & (df['price']<= 1500000)]                
    elif price == price_list[5]:
        return df[(df['year']==eval(year)) & (df['price']<= 1500000)]
    elif price == price_list[6] and year == year_list[0]:
        return df[(df['year']<=2000) & (df['price']<= 2000000)]                
    elif price == price_list[6]:
        return df[(df['year']==eval(year)) & (df['price']<= 2000000)]
    elif price == price_list[7] and year == year_list[0]:
        return df[(df['year']<=2000) & (df['price']<= 3000000)]                
    elif price == price_list[7]:
        return df[(df['year']==eval(year)) & (df['price']<= 3000000)]
...
def sort_by_price(price):
    if price == price_list[0]:
        ...
        return df[df['price']<= 100000]
    elif price == price_list[3]:
        return df[df['price']<= 500000]
    elif price ==price_list[4]:
        return df[df['price']<= 1000000]
    elif price == price_list[5]:
        return df[df['price']<= 1500000]
    elif price ==price_list[6]:
        return df[df['price']<= 2000000]
    elif price == price_list[7]:
        return df[df['price']<= 3000000]   
    elif price == price_list[8]:
        return df[df['price']> 3000000]
...
label_img = tk.Label(frame0, image=tk_img, width=500, height=100, bg="orange")
label_img.pack()
...
button5 = tk.Button(frame4, text="輸出圖表\n年份／公里數／價格", font=font3,command=to_png)
