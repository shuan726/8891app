...
def sort_by_year_price(year,price):
    if price == price_list[0] and year == year_list[0]:
        df[df['year']<=2000].sort_values(['price'],ascending = False)
    elif price == price_list[1] and year == year_list[0]:
        df[df['year']<=2000].sort_values(['price'])
    elif price == price_list[0]:
        df[df['year']==eval(year)].sort_values(['price'],ascending = False)
    elif price == price_list[1]:
        df[df['year']==eval(year)].sort_values(['price'])
    elif price == price_list[2] and year == year_list[0]:
        df[(df['year']<=2000) & (df['price']<= 100000)]
    elif price == price_list[2]:
        df[(df['year']==eval(year)) & (df['price']<= 100000)]
    elif price == price_list[3] and year == year_list[0]:
        df[(df['year']<=2000) & (df['price']<= 500000)]                
    elif price == price_list[3]:
        df[(df['year']==eval(year)) & (df['price']<= 500000)]
    elif price == price_list[4] and year == year_list[0]:
        df[(df['year']<=2000) & (df['price']<= 1000000)]                
    elif price == price_list[4]:
        df[(df['year']==eval(year)) & (df['price']<= 1000000)]
    elif price == price_list[5] and year == year_list[0]:
        df[(df['year']<=2000) & (df['price']<= 1500000)]                
    elif price == price_list[5]:
        df[(df['year']==eval(year)) & (df['price']<= 1500000)]
    elif price == price_list[6] and year == year_list[0]:
        df[(df['year']<=2000) & (df['price']<= 2000000)]                
    elif price == price_list[6]:
        df[(df['year']==eval(year)) & (df['price']<= 2000000)]
    elif price == price_list[7] and year == year_list[0]:
        df[(df['year']<=2000) & (df['price']<= 3000000)]                
    elif price == price_list[7]:
        df[(df['year']==eval(year)) & (df['price']<= 3000000)]
...
list_var.set([])
price_list=['由高至低↓','由低至高↑','10萬↓','50萬↓','100萬↓','150萬↓','200萬↓','300萬↓']
year_list=['2000年↓']
...
button0 = tk.Button(frame1, text="查詢", font=font5,fg = 'black',command = lambda:[search_text(),find_car()])
